...
//		return connection.hasStatement(uri, RDF.TYPE, ValueFactoryImpl.getInstance().createURI(STEP.StartAPI.getLabel()), true, new Resource[0]);
	...
	@Override
	public void writeResource(final URI baseUri, Iterable<org.semanticweb.yars.nx.Node[]> nodes, OutputStream output, RDFFormat format) throws RDFHandlerException {

		final RDFWriter writer = Rio.createWriter(format, output);

		List<Statement> iterable = new LinkedList<Statement>();

		ValueFactory factory = ValueFactoryImpl.getInstance();
		for (org.semanticweb.yars.nx.Node[] node : nodes) {

			URI subject = null;
			try {
				subject = factory.createURI(node[0].getLabel());
			} catch (IllegalArgumentException e) {
				// relative URIs are difficult for the writer
				subject = factory.createURI(baseUri.getNamespace() + node[0].getLabel());
			}

			URI predicate = factory.createURI(node[1].getLabel());

			Value object = null;

			try {
				if (node[2] instanceof org.semanticweb.yars.nx.Resource) {
					object = factory.createURI(node[2].getLabel());
				} else {
					object = factory.createLiteral(node[2].getLabel());
				}
			} catch (IllegalArgumentException e) {
				// relative URIs are difficult for the writer
				object = factory.createURI(baseUri.getNamespace() + node[2].getLabel());
			}

			iterable.add( factory.createStatement(subject, predicate, object ));

		}


		Iterator<Statement> iter = iterable.iterator();
		final CloseableIteration<Statement, RepositoryException> statements = new CloseableIteratorIteration<Statement, RepositoryException>(iter);


		try {
			LdpUtils.exportIteration(writer, baseUri, statements);
		} catch (RepositoryException e) {
			log.error("Problem writing result to response body output stream: ", e);
			e.printStackTrace();
